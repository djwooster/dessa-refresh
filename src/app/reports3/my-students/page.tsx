    ...
    <div className="pt-16 px-6 pb-6 space-y-5">
      ...
      <div className="bg-white rounded-xl border border-[#e8ecf0] shadow-sm">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#e0e5eb] bg-[#f4f6f8]">
              {["Name", "Username", "SSR Points", "Descriptive Range", "T-Score", "Rating Form", "Rating Window", "Rating Date"].map((col) => (
                <th key={col} className="px-4 py-3 text-left text-[12px] font-semibold tracking-wide text-gray-700 whitespace-nowrap">
                  {col}
                </th>
              ))}
            </tr>
            ...
            {paginatedStudents.map((s, i) => {
              ...
              return (
                <tr key={s.id} className={`hover:bg-gray-50/70 transition-colors ${!isLast ? "border-b border-[#f5f7fa]" : ""}`}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center h-5 px-2 rounded-md text-[12px] font-medium ${LEVEL_BADGE[s.range]}`}>
                      {s.range}
                    </span>
                  </td>
